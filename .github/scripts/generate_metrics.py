...
def unavailable_eval_badge() -> Dict[str, Any]:
    """Return an explicit unavailable eval badge when a real score is absent."""
    return {
        "schemaVersion": 1,
        "label": "Eval Score",
        "message": "N/A",
...
def calculate_coverage(coverage_dir: Path) -> Dict[str, Any]:
    ...
    if valid_coverage_files:
        ...
        raise RuntimeError(
            "No valid coverage data found; refusing to overwrite existing coverage metrics with a fallback value"
        )
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if match:
                ...
                print("Eval script succeeded but no parseable score was found", file=sys.stderr)
                return unavailable_eval_badge()
        else:
...
def main(argv=None):
    ...
    try:
        ...
        print(f"Metric generation failed: {e}", file=sys.stderr)
        return 1
