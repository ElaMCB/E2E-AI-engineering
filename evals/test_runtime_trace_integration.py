...
def test_zero_previous_week_activity_does_not_fail_change_detection(tmp_path):
    """A quiet previous week followed by activity should not divide by zero."""
    analyzer_module = _load_analyzer_module()
    results_dir = tmp_path / "results"
    results_dir.mkdir(parents=True, exist_ok=True)
    ...
    assert any(
        ...
        and "Activity resumed after a quiet week" in change["description"]
...
def test_agent_failures_do_not_overwrite_latest_analysis(tmp_path):
    """Failed agents should produce a trace but not publish partial analysis."""
    analyzer_module = _load_analyzer_module()
    results_dir = tmp_path / "results"
    results_dir.mkdir(parents=True, exist_ok=True)
    ...
    existing_analysis = {"date": "2026-04-13", "executive_summary": {"summary": "previous"}}
    ...
    analyzer.summary_agent.run = fail_summary
    analysis = analyzer.analyze_weekly_data()

    assert analysis == {"error": "Analysis failed for agent(s): summarization"}
    with open(results_dir / "latest_analysis.json", "r", encoding="utf-8") as f:
        assert json.load(f) == existing_analysis

    with open(results_dir / "latest_runtime_trace.json", "r", encoding="utf-8") as f:
        trace_data = json.load(f)
    assert trace_data["summary"]["failed_steps"] == 1
    assert any(run["agent_name"] == "summarization" and run["status"] == "error" for run in trace_data["runs"])
