...
def test_analyzer_handles_activity_spike_after_quiet_week(tmp_path):
    """A zero-update prior week should not hide change detection behind a runtime error."""
    analyzer_module = _load_analyzer_module()
    results_dir = tmp_path / "results"
    results_dir.mkdir(parents=True, exist_ok=True)
    ...
    assert change_detection_runs
    assert change_detection_runs[0]["status"] == "ok"
