...
def test_analyzer_handles_activity_after_empty_week(tmp_path):
    """An empty previous week followed by activity should not fail change detection."""
    analyzer_module = _load_analyzer_module()
    results_dir = tmp_path / "results"
    results_dir.mkdir(parents=True, exist_ok=True)
    ...
    assert activity_changes
    assert activity_changes[0]["week_over_week_change"] == {"prev": 0, "current": 1}
    assert "new activity after quiet week" in activity_changes[0]["description"]
