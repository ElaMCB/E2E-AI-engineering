...
def test_change_detection_handles_activity_after_quiet_week(tmp_path):
    """A previous week with zero updates should not crash change detection."""
    analyzer_module = _load_analyzer_module()
    results_dir = tmp_path / "results"
    results_dir.mkdir(parents=True, exist_ok=True)
...
def test_analyzer_preserves_previous_analysis_when_agent_fails(tmp_path):
    """Partial agent output must not overwrite the last known-good analysis."""
    analyzer_module = _load_analyzer_module()
    results_dir = tmp_path / "results"
    results_dir.mkdir(parents=True, exist_ok=True)

    with open(results_dir / "latest_analysis.json", "w", encoding="utf-8") as f:
        json.dump({"date": "previous", "top_insights": ["keep"]}, f)
    ...
    assert analysis == {"error": "Analysis failed in agent(s): change_detection"}
    with open(results_dir / "latest_analysis.json", "r", encoding="utf-8") as f:
        assert json.load(f) == {"date": "previous", "top_insights": ["keep"]}
