...
def test_change_detection_handles_zero_previous_week(tmp_path):
    """A quiet previous week followed by new activity must not crash analysis."""
    analyzer_module = _load_analyzer_module()
    results_dir = tmp_path / "results"
    results_dir.mkdir(parents=True, exist_ok=True)
...
def test_agent_failure_preserves_previous_analysis_and_records_trace(tmp_path):
    """Failed agents should not overwrite latest_analysis.json with partial data."""
    analyzer_module = _load_analyzer_module()
    results_dir = tmp_path / "results"
    results_dir.mkdir(parents=True, exist_ok=True)
