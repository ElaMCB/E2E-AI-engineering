...
def test_change_detection_handles_activity_from_zero(tmp_path):
    """Activity after an empty week should not fail the runtime or hide the spike."""
    analyzer_module = _load_analyzer_module()
    results_dir = tmp_path / "results"
    results_dir.mkdir(parents=True, exist_ok=True)
    ...
    change_categories = {change["category"] for change in analysis["significant_changes"]}
    assert "market_shift" in change_categories
