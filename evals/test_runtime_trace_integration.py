...
def test_zero_to_active_week_does_not_fail_runtime(tmp_path):
    """A quiet week followed by activity should not divide by zero."""
    analyzer_module = _load_analyzer_module()
    results_dir = tmp_path / "results"
    results_dir.mkdir(parents=True, exist_ok=True)
    ...
    assert analysis["significant_changes"][0]["category"] == "new_company"
    assert any(
...
def test_agent_failure_preserves_previous_analysis(tmp_path):
    """Failed agent runs should emit a trace without overwriting latest analysis."""
    analyzer_module = _load_analyzer_module()
    results_dir = tmp_path / "results"
    results_dir.mkdir(parents=True, exist_ok=True)

    previous_analysis = {"date": "2026-04-13", "executive_summary": {"summary": "previous"}}
    ...
    def fail_analysis(_updates, _historical_data):
        raise RuntimeError("simulated critical agent failure")

    analyzer.summary_agent.analyze = fail_analysis
    analysis = analyzer.analyze_weekly_data()

    assert analysis == {"error": "Analysis failed in agent(s): summarization"}
