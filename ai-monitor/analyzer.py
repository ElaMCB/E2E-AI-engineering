        ...
        if len(historical_data) >= 2:
            ...
            if prev_count == 0 and curr_count > 0:
                changes.append(Change(
                    ...
                    description=f"Activity resumed after no updates last week: {prev_count} → {curr_count} updates",
                    magnitude=6.0,
                    week_over_week_change={'prev': prev_count, 'current': curr_count},
                    implications=["New activity after a quiet week - monitor closely"]
                ))
...
class IntelligentAnalyzer:
    ...
    def analyze_weekly_data(self) -> Dict:
        ...
        if failed_runs:
            ...
            errors = "; ".join(f"{run.agent_name}: {run.error}" for run in failed_runs)
            return {"error": f"Analysis failed; latest_analysis.json was not updated. {errors}"}
