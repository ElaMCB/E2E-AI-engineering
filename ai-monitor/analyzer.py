    ...
    def analyze(self, updates: List[Dict], historical_data: List[Dict]) -> List[Change]:
        ...
        if len(historical_data) >= 2:
            ...
            if curr_count > prev_count * 1.5:  # 50% increase
                if prev_count == 0:
                    change_description = f"Significant activity increase: 0 → {curr_count} updates (new activity)"
                else:
                    change_description = f"Significant activity increase: {prev_count} → {curr_count} updates (+{((curr_count/prev_count-1)*100):.0f}%)"
