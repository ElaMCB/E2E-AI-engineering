    ...
    def analyze(self, updates: List[Dict], historical_data: List[Dict]) -> List[Change]:
        ...
        if len(historical_data) >= 2:
            ...
            if curr_count > prev_count * 1.5:  # 50% increase
                if prev_count == 0:
                    increase_label = "new activity from zero baseline"
                else:
                    increase_label = f"+{((curr_count / prev_count - 1) * 100):.0f}%"

                changes.append(Change(
                    category='market_shift',
                    description=f"Significant activity increase: {prev_count} → {curr_count} updates ({increase_label})",
                    magnitude=6.0,
