        ...
        if len(historical_data) >= 2:
            ...
            if prev_count == 0 and curr_count > 0:
                changes.append(Change(
                    ...
                    description=f"Activity resumed after quiet previous week: {prev_count} → {curr_count} updates",
                    magnitude=6.0,
                    week_over_week_change={'prev': prev_count, 'current': curr_count},
                    implications=["Monitor resumed market activity closely", "Review new updates for major announcements"]
                ))
