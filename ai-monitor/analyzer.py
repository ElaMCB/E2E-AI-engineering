    ...
    def analyze(self, updates: List[Dict], historical_data: List[Dict]) -> List[Change]:
        ...
        if len(historical_data) >= 2:
            ...
            if curr_count > prev_count * 1.5:  # 50% increase
                if prev_count == 0:
                    change_description = f"Activity resumed after quiet week: {curr_count} updates"
                    percentage_change = None
                else:
                    percentage_change = (curr_count / prev_count - 1) * 100
                    change_description = f"Significant activity increase: {prev_count} → {curr_count} updates (+{percentage_change:.0f}%)"

                changes.append(Change(
                    category='market_shift',
                    description=change_description,
                    magnitude=6.0,
                    week_over_week_change={'prev': prev_count, 'current': curr_count, 'percentage_change': percentage_change},
...
class IntelligentAnalyzer:
    ...
    def analyze_weekly_data(self) -> Dict:
        ...
        if failed_runs:
            ...
            return {"error": f"Analysis failed for agent(s): {failed_agents}"}
