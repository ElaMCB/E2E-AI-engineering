        ...
        if len(historical_data) >= 2:
            ...
            if prev_count == 0 and curr_count > 0:
                changes.append(Change(
                    ...
                    description=f"Activity resumed after quiet week: {prev_count} → {curr_count} updates",
                    magnitude=6.0,
                    week_over_week_change={'prev': prev_count, 'current': curr_count},
                    implications=["New market activity after a quiet week - review fresh developments"]
                ))
...
class IntelligentAnalyzer:
    ...
    def analyze_weekly_data(self) -> Dict:
        ...
        if failed_runs:
            ...
            failed_steps = ", ".join(f"{run.agent_name}: {run.error}" for run in failed_runs)
            return {"error": f"Analysis failed in agent runtime: {failed_steps}"}
