        ...
        if len(historical_data) >= 2:
            ...
            if prev_count == 0 and curr_count > 0:
                changes.append(Change(
                    ...
                    description=f"Activity resumed after a quiet week: {curr_count} updates after 0 the prior week",
                    magnitude=6.0,
                    week_over_week_change={'prev': prev_count, 'current': curr_count},
                    implications=["New market activity after a quiet week - monitor closely"]
                ))
...
class IntelligentAnalyzer:
    ...
    def analyze_weekly_data(self) -> Dict:
        ...
        if agent_errors:
            ...
            error_details = "; ".join(
                f"{run.agent_name}: {run.error or run.status}" for run in agent_errors
            )
            return {"error": f"Analysis agent failure(s): {error_details}"}
