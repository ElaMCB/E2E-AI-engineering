        ...
        if len(historical_data) >= 2:
            ...
            if prev_count == 0 and curr_count > 0:
                changes.append(Change(
                    ...
                    description=f"Activity resumed after a quiet week: 0 → {curr_count} updates",
                    magnitude=6.0,
                    week_over_week_change={'prev': prev_count, 'current': curr_count},
                    implications=["Market activity resumed - monitor closely", "Potential major announcements coming"]
                ))
...
class IntelligentAnalyzer:
    ...
    def analyze_weekly_data(self) -> Dict:
        ...
        failed_agents = [
            run.agent_name for run in self.runtime.run_history
            if run.status != "ok"
        ]
        if failed_agents:
            return {
                "error": (
                    "Analysis failed; preserving the previous analysis. "
