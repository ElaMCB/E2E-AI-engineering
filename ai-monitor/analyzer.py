    ...
    def analyze(self, updates: List[Dict], historical_data: List[Dict]) -> List[Change]:
        ...
        if len(historical_data) >= 2:
            ...
            if curr_count > prev_count * 1.5:  # 50% increase
                if prev_count:
                    increase_description = (
                        f"Significant activity increase: {prev_count} → {curr_count} updates "
                        f"(+{((curr_count / prev_count - 1) * 100):.0f}%)"
                    )
                else:
                    increase_description = (
                        f"Significant activity increase: {prev_count} → {curr_count} updates "
                        "(new activity after quiet week)"
                    )
                changes.append(Change(
...
class IntelligentAnalyzer:
    ...
    def analyze_weekly_data(self) -> Dict:
        ...
        if failed_runs:
            ...
            failed_agents = ", ".join(
                f"{run.agent_name}: {run.error or 'unknown error'}" for run in failed_runs
            )
            return {"error": f"Analysis agents failed: {failed_agents}"}
